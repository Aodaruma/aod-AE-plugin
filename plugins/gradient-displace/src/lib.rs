...
const PLUGIN_DESCRIPTION: &str =
    "Applies directional displacement along image gradients or hue-saturation vectors.";
        ...
        match cmd {
            ae::Command::About => {
                out_data.set_return_msg(
                    format!(
                        "AOD_GradientDisplace - {version}\r\r{PLUGIN_DESCRIPTION}\rCopyright (c) 2026-{build_year} Aodaruma",
...
impl Plugin {
    ...
    fn do_render(
        ...
        let distance = params.get(Params::Distance)?.as_float_slider()?.value() as f32;
        ...
        out_layer.iterate(0, progress_final, None, |x, y, mut dst| {
            ...
            let out_px = if !flow.valid || distance == 0.0 || strength <= 0.0 {
                center
            } else {
                ...
                let shift = distance * strength * flow_strength;
                let sx = x as f32 + flow.dir_x * shift;
                let sy = y as f32 + flow.dir_y * shift;
                sample_bilinear(&src, w, h, sx, sy, edge_mode)
            };
