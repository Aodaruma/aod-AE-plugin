...
- 公開版の緊急修正（hotfix）は最新 `origin/main` から作成し、`main` へ PR を出す。取り込み後は `main` → `dev` の同期 PR で開発版へ戻す。
- 通常リリースは検証済みの `dev` → `main` の PR で行う。`main` / `dev` 間の統合には Merge commit を使い、Squash / Rebase merge は使わない。
- 同期時の競合や修正単位で取り込む例外は [ブランチ運用](docs/development.md#ブランチ運用) を参照する。
